...
-}
data IlaConfig dom (out :: Ports) where
  IlaConfig ::
...
-}
data WithIlaConfig dom a (out :: Ports) where
  WithIlaConfig ::
    forall dom n m a (out :: Ports).
    ( KnownNat n
    , KnownNat m
    , BitPack a
    , 1 <= BitSize a `DivRU` 32
...
{- | Terminating case
Once encountering a `WithIlaConfig`, terminate the collecting of signals and return the `IlaConfig`
generated by the collected signals and labels.
-}
instance
  forall dom0 dom1 n a b (out0 :: Ports) (out1 :: Ports).
  ( NFDataX a
  , dom0 ~ dom1
  , BitPack a
  , 1 <= BitSize a `DivRU` 32
...
signalInfoBBF _ _ args _ = view tcCache >>= go
 where
  ...
  -- \| Get the signal information from its AST form
...
-- | Type-level list for describing ports
type Ports = [(Data.Kind.Type, Symbol)]

-- | Compute the length of `Ports`
type family CountPorts (o :: Ports) :: Nat where
  CountPorts '[] = 0
  CountPorts (_ ': xs) = 1 + (CountPorts xs)

-- | Constrain @a@ in each @'(a, symbol)@ of `Ports`
type family
  ConstrainedPorts (o :: Ports) (c :: Data.Kind.Type -> Data.Kind.Constraint) ::
    Data.Kind.Constraint
  where
  ConstrainedPorts '[] c = ()
  ConstrainedPorts ('(a, _) ': xs) c = (c a, ConstrainedPorts xs c)

-- | Associates `Ports` with a tuple type and provides a function for obtaining the JSON representation for each port.
class
...
instance
  ...
  KnownPorts ('(a, name) ': xs)
  where
  type PortsTuple ('(a, name) ': xs) = (a, PortsTuple xs)
