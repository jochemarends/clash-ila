...
-}
data IlaConfig dom (out :: OutSigList) where
  IlaConfig ::
    forall dom a out n m.
    ( KnownNat n
    , KnownNat m
    , NFDataX a
    , BitPack a
...
-}
data WithIlaConfig dom a (out :: OutSigList) where
  WithIlaConfig ::
    forall dom n m a (out :: OutSigList).
    ( KnownNat n
    , KnownNat m
    , BitPack a
    , 1 <= BitSize a `DivRU` 32
    ...
    -- ^ A list of predicates, used to control trigger/capture logic
    , outputs :: Proxy out
    -- ^ A type level list representing the output signals to emit
    } ->
    WithIlaConfig dom a out
...
-}
instance
  forall dom0 dom1 n a b (out0 :: OutSigList) (out1 :: OutSigList).
  ( NFDataX a
  , dom0 ~ dom1
  , BitPack a
  , 1 <= BitSize a `DivRU` 32
  , a ~ b
  , KnownOutSigList out0
  , out0 ~ out1
  ) =>
  LabeledSignals
  ...
  ilaProbe (signalInfos, tracing) (WithIlaConfig @_ @_ @_ @_ @_ toplevel triggerPoint bufferDepth predicates _outputs) =
   ...
   where
    ilaHash = writeSignalInfo toplevel bufferDepth (fromGenSignal <$> signalInfos) (fromGenSignal <$> outSigListToGenSignals @out0) (snd <$> predicates)
...
writeSignalInfo ::
  ...
  BitVector 32
writeSignalInfo !_toplevel !_bufSize !_sigInfo !_outputs !_triggerNames = 0
...
signalInfoBBF _ _ args _ = view tcCache >>= go
 where
  ...
  getGenIla ::
    ...
    Vec m (String) ->
    GenIla
  getGenIla toplevel bufSize sigInfo outputs triggerNames =
    GenIla
...
-- | Type-level list for describing output signals
type OutSigList = [(Data.Kind.Type, Symbol)]

-- | Compute the length of an `OutSigList`
type family OutSigListLength (o :: OutSigList) :: Nat where
  OutSigListLength '[] = 0
  OutSigListLength (_ ': xs) = 1 + (OutSigListLength xs)

-- | Constrain @a@ in each @'(a, symbol)@ of an `OutSigList`
type family
  OutSigListConstrained (o :: OutSigList) (c :: Data.Kind.Type -> Data.Kind.Constraint) ::
    Data.Kind.Constraint
  where
  OutSigListConstrained '[] c = ()
  OutSigListConstrained ('(a, _) ': xs) c = (c a, OutSigListConstrained xs c)

-- | Associates an `OutSigList` with a tuple type and provides a function for obtaining the JSON representation for each signal.
class
  ( OutSigListConstrained o BitPack
  , OutSigListConstrained o NFDataX
  , OutSigListConstrained o Generic
  , BitPack (OutSigTuple o)
  , NFDataX (OutSigTuple o)
  , Generic (OutSigTuple o)
  ) =>
  KnownOutSigList (o :: OutSigList)
  where
  type OutSigTuple o :: Data.Kind.Type
  outSigListToGenSignals :: Vec (OutSigListLength o) GenSignal

instance KnownOutSigList '[] where
  type OutSigTuple '[] = ()
  outSigListToGenSignals = Nil
