...
-}
data IlaConfig dom out where
  IlaConfig ::
    forall dom a out n m l.
    ( KnownNat n
    , KnownNat m
    , KnownNat l
    , NFDataX a
    ...
    , outputs :: (Vec l GenSignal, Proxy out)
    } ->
    IlaConfig dom out
...
-}
data WithIlaConfig dom a out where
  WithIlaConfig ::
    forall dom n m l a out.
    ( KnownNat n
    , KnownNat m
    , KnownNat l
    , BitPack a
...
-}
instance
  forall dom0 dom1 n a b out out1.
  ( NFDataX a
  , dom0 ~ dom1
  , BitPack a
  , 1 <= BitSize a `DivRU` 32
  ...
  LabeledSignals
    ((Vec n GenSignal, Signal dom0 a) -> WithIlaConfig dom1 b out -> IlaConfig dom0 out1)
  where
  ilaProbe ::
    (Vec n GenSignal, Signal dom0 a) ->
    WithIlaConfig dom1 a out ->
    IlaConfig dom0 out1
  ilaProbe (signalInfos, tracing) (WithIlaConfig @_ @_ @_ @_ toplevel triggerPoint bufferDepth predicates outputs) =
    IlaConfig
      { depth = bufferDepth
   ...
   where
    ilaHash = writeSignalInfo toplevel bufferDepth (fromGenSignal <$> signalInfos) (fromGenSignal <$> fst outputs) (snd <$> predicates)
 ...
 where
  ...
  getGenIla toplevel bufSize sigInfo outputs triggerNames =
    GenIla
      ...
      , outputs = toList $ map (\(w, n) -> GenSignal { name = n, width = fromIntegral w }) outputs
      }
...
class LabeledOutputs a where
  ilaOuts :: a

instance
  forall a b m n next.
  ( m ~ n + 1
  , LabeledOutputs ((Vec m GenSignal, Proxy (a, b)) -> next)
  , KnownNat (BitSize b)
  ) =>
  LabeledOutputs ((Vec n GenSignal, Proxy a) -> (Proxy b, String) -> next)
  where
  ilaOuts :: (Vec n GenSignal, Proxy a) -> (Proxy b, String) -> next
  ilaOuts (prevInfos, prevProxy) (newProxy, newName) =
    ilaOuts (prevInfos ++ (newInfo :> Nil), newBundled)
   where
    newInfo = GenSignal{name = newName, width = natToNum @(BitSize b)}
    newBundled = (,) <$> prevProxy <*> newProxy

instance
  forall a n.
  LabeledOutputs ((Vec n GenSignal, Proxy a) -> (Vec n GenSignal, Proxy a))
  where
  ilaOuts :: (Vec n GenSignal, Proxy a) -> (Vec n GenSignal, Proxy a)
  ilaOuts = id

ilaOutputs ::
  forall a next.
  ( LabeledOutputs ((Vec 1 GenSignal, Proxy ((), a)) -> next)
    , KnownNat (BitSize a)
  ) =>
  (Proxy a, String) -> next
ilaOutputs (o :: (Proxy a, String)) =
  ilaOuts (Nil :: Vec 0 GenSignal, Proxy @()) o
