...
pub struct State<'a> {
    signals: &'a [IlaSignal],
    value_states: Vec<TextPromptState<()>>,
...
impl State<'_> {
    ...
    /// to zero.
    pub fn new(ila: &'_ IlaConfig) -> State<'_> {
        State {
            signals: &ila.outputs,
            value_states: ila.outputs.iter().map(|_| TextPromptState::new(Some("0"), ())).collect(),
            cursor_position: 0,
        }
    }
    ...
    pub fn render(&mut self, terminal: &mut Terminal<CrosstermBackend<Stdout>>) {
        let _ = terminal.draw(|f| {
            ...
            for (index, signal) in self.signals.iter().enumerate() {
                ...
                let prompt = &mut self.value_states[index];
    ...
    where
        T: Read + Write {
        // This function is copied from the predicate TUI. Maybe make it globally scoped to prevent
        // duplication?
        ...
        // When serializing a cluster the signals get reversed. Reverse the iterator here to
        // preserve the order.
        let signals: Vec<Signal> = self.signals
            .iter()
            .zip(&self.value_states)
            .filter_map(|(s, state)| {
                NumericState::immediate_parse(&state.input)
        ...
        let _ = perform_register_operation(
            medium,
            ila,
            &IlaRegisters::StagedOutput(cluster.to_data()),
        );

        let _ = perform_register_operation(medium, ila, &IlaRegisters::CommitOutput);
    }

    fn handle_input(&mut self, event: &Event) {
        if let Event::Key(KeyEvent { code, .. }) = event {
            if let Some(ref mut prompt) = self.value_states.get_mut(self.cursor_position) {
